...
RunWaitOutput(command, vSize:="")
{
    ...
    if (!vPID) {
        ...
        static oShell := ComObject("WScript.Shell")
    }
    oExec := oShell.Exec(A_ComSpec " /c " command)
    vStdOut := ""
    if !(vSize = "")
        VarSetStrCapacity(&vStdOut, vSize)
    while !oExec.StdOut.AtEndOfStream
        vStdOut := oExec.StdOut.ReadAll()
    return RTrim(vStdOut, "`r`n")
}
...
Query(state_query) {
    return RunWaitOutput("komorebic.exe query " state_query)
}
